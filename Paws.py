...
class PAWS:
    ...
    def welcome(self):
        print(
        f"""
    {Fore.RED + Style.BRIGHT}       PAWS BOT     
    ╔═╗┬  ┌─┐┌┐┌┌─┐┬
    ╠═╝│  ├┤ │││└─┐│
    ╩  ┴─┘└─┘┘└┘└─┘┴{Style.RESET_ALL}

    {Fore.RED + Style.BRIGHT}PLENSI{Style.RESET_ALL} {Fore.BLACK + Style.BRIGHT} PLANC COMMUNITY BOT{Style.RESET_ALL}
    {Fore.YELLOW + Style.BRIGHT}JOIN GROUP{Style.RESET_ALL}   : {Fore.LIGHTYELLOW_EX}https://t.me/plancnewss{Style.RESET_ALL}
    {Fore.YELLOW + Style.BRIGHT}JOIN DISCUSS{Style.RESET_ALL} : {Fore.LIGHTYELLOW_EX}https://t.me/plancinsiderss{Style.RESET_ALL}
        """
    )
    ...
    def user_auth(self, query: str, retries=5):
        ...
        data = json.dumps({'data':query, 'referralCode':'MzHf8z6O'})
        self.headers.update({
                    ...
                    for quest in quests:
                        ...
                        if quest and not claimed:
                            ...
                            if current_progress != total_progress:
                                ...
                                if start:
                                    self.log(
                                        f"{Fore.CYAN+Style.BRIGHT}[ Quest{Style.RESET_ALL}"
    ...
    def show_menu(self):
        ...
        print(f"{Fore.CYAN + Style.BRIGHT}║{Style.RESET_ALL}{Fore.WHITE + Style.BRIGHT}        PAWS MENU        {Style.RESET_ALL}{Fore.CYAN + Style.BRIGHT}║{Style.RESET_ALL}")
        print(f"{Fore.CYAN + Style.BRIGHT}╠══════════════════════════╣{Style.RESET_ALL}")
        print(f"{Fore.CYAN + Style.BRIGHT}║{Style.RESET_ALL} 1. Start Auto Quest      {Fore.BLUE + Style.BRIGHT}║{Style.RESET_ALL}")
        print(f"{Fore.CYAN + Style.BRIGHT}║{Style.RESET_ALL} 2. Show Account Balance  {Fore.BLUE + Style.BRIGHT}║{Style.RESET_ALL}")
        print(f"{Fore.CYAN + Style.BRIGHT}║{Style.RESET_ALL} 3. Exit                  {Fore.BLUE + Style.BRIGHT}║{Style.RESET_ALL}")
        print(f"{Fore.CYAN + Style.BRIGHT}╚══════════════════════════╝{Style.RESET_ALL}\n")

    def show_account_balance(self, queries):
        self.clear_terminal()
        self.welcome()
        self.log(f"{Fore.GREEN + Style.BRIGHT}Checking Account Balances...{Style.RESET_ALL}")
        self.log(f"{Fore.CYAN + Style.BRIGHT}-{Style.RESET_ALL}"*75)
        ...
        input(f"\n{Fore.YELLOW + Style.BRIGHT}Press Enter to return to menu...{Style.RESET_ALL}")

    def start_auto_quest(self, queries):
        self.clear_terminal()
        self.welcome()
        self.log(
            f"{Fore.GREEN + Style.BRIGHT}Account's Total: {Style.RESET_ALL}"
    ...
    def main(self):
        ...
        try:
            ...
            while True:
                ...
                choice = input(f"{Fore.YELLOW + Style.BRIGHT}Enter your choice (1-3): {Style.RESET_ALL}")
                
                if choice == "1":
                    self.start_auto_quest(queries)
                elif choice == "2":
                    self.show_account_balance(queries)
                elif choice == "3":
                    self.log(f"{Fore.RED + Style.BRIGHT}[ EXIT ] PAWS - BOT{Style.RESET_ALL}")
                    break
                else:
                    self.log(f"{Fore.RED + Style.BRIGHT}Invalid choice. Please try again.{Style.RESET_ALL}")
                    time.sleep(2)
